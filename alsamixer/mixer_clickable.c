...
#if 0
void clickable_begin(WINDOW *win, command_enum command, int arg1) {
	int x, y;
	getyx(win, y, x);
	clickable_set(y, x, 0, 0, command, arg1);
}

void clickable_end(WINDOW *win) {
	if (! clickable_rects_count)
		return;

	getyx(win, clickable_rects[last_rect].y2, clickable_rects[last_rect].x2);
	clickable_rects[last_rect].x2--;
}
#endif
